...
oddFactorial 1 = 1
oddFactorial n = if mod n 2 == 0 then oddFactorial (n-1) else n * oddFactorial (n-1)
...
leftpad s 0 = s
leftpad s b = if length s > b then s else addPadding s (b - length s)
    where addPadding s 0 = s
          addPadding s p = addPadding (" " ++ s) (p-1)
...
countdown n = "Ready! " ++ (countPrinter n) ++ "Liftoff!"
    where countPrinter 1 = "1... "
          countPrinter n = show n ++ "... " ++ countPrinter (n-1)
...
smallestDivisor n = finder 2 n

finder :: Integer -> Integer -> Integer
finder d n 
  | mod n d == 0  = d
  | otherwise     = finder (d+1) n
...
isPrime n = if n > 1 && smallestDivisor n == n then True else False
